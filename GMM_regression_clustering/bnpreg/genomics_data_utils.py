# copied over from 
# https://github.com/rgiordan/AISTATS2019SwissArmyIJ/blob/master/genomics/aistats2019_ij_paper/loading_data_utils.py

# the as_matrix method is outdated; replacted with to_numpy()
...
from bnpreg.genomics_utils import \
...
def _load_raw_data(
    ...
    meta = which_to_keep
    gene_names = X.index
    timepoints = which_to_keep.time.to_numpy().astype(float)
    ...
    if filter_de_genes_from is not None:
        ...
        X = X.loc[which_to_keep.index].to_numpy().astype(float)
